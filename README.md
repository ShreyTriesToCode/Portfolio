...
These migrations safely add missing project sync columns, create `github_project_sync_logs`, create a Supabase-backed `certifications` table, enable RLS, and add public read policies for portfolio display.

## Supabase Certifications

The Experience/Certifications section reads from the `certifications` table. Add rows in Supabase Table Editor with at least:
