...
export default function VscodeShell() {
  ...
  const commands = [
    ...
    { title: "Go to Certifications", Icon: BadgeCheck, action: () => openFile("certifications.md") },
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      setSbState("checking");
      const { error } = await withTimeout(supabase.from("projects").select("id").limit(1)).catch((error) => ({ error }));
      if (cancelled) return;
  ...
  useEffect(() => {
    ...
    async function loadShellData() {
      ...
      try {
        const [{ data: profile }, { data: projects }, syncStatusResponse] = await Promise.all([
          ...
          withTimeout(fetch("/api/sync-status", { cache: "no-store" }).then((response) => response.json() as Promise<GithubSyncStatus>)),
        ]);
        ...
        if (!syncStatusResponse) {
          ...
          setSyncStatus({
            status: syncStatusResponse.status === "failed" ? "failed" : syncStatusResponse.status === "synced" ? "synced" : "pending",
            started_at: syncStatusResponse.started_at ?? "",
            finished_at: syncStatusResponse.finished_at ?? "",
            repos_scanned: Number(syncStatusResponse.repos_scanned ?? 0),
            repos_imported: Number(syncStatusResponse.repos_imported ?? 0),
            repos_skipped: Number(syncStatusResponse.repos_skipped ?? 0),
          });
        }
      } catch (error) {
        if (cancelled) return;
      ...
      >
        ...
        <div className="flex min-w-max flex-1 items-center justify-end gap-2 sm:gap-3">
          ...
          <span
            ...
            title={
              syncStatus.status === "failed"
                ? "Last GitHub sync failed. Check GitHub Actions or the local sync script for details."
                : undefined
            }
          >
