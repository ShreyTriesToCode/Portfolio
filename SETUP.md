...
- Public users can insert contact messages but cannot read them.
- Detailed GitHub sync logs are server-only; the site reads sanitized sync status through `/api/sync-status`.
...
After adding a row, refresh the portfolio and open the Certifications section.
...
11. Open Certifications and confirm certifications load.
